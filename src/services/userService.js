...
let handleAdminLogin = (email, password) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (isExist) {
                const [rows] = await pool.execute('SELECT * FROM account where email=? and role=1', [email])
                // console.log('>>>CHeck', password, rows[0].password);
...
let checkAdminEmail = (userEmail) => {
    return new Promise(async (resolve, reject) => {
        try {
            const [rows] = await pool.execute('SELECT count(*) as count FROM account where email=? and role=1', [userEmail])
            console.log({ ...rows[0] });
            let check = rows[0].count > 0
            if (check) {
                resolve(true)
